...
1. 填写`基本信息`，进入`容器配置`输入如下信息。
    ...
    - `网络资源参数`：
        ...
        - 基于 Macvlan/VLAN CNI 使用 RDMA 时，资源名称为 [RDMA 安装及使用准备](rdmapara.md//#_1) 中创建 Spiderpool 时自定义名称。

        - 基于 SRIOV CNI 使用 RDMA 时，资源名称为 `SriovNetworkNodePolicy` 中定义的 `resourceName` 。详情参考 [RDMA 安装及使用准备](rdmapara.md/#_2)。
